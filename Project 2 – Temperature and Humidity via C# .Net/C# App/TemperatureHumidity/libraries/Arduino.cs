...
namespace LattePanda.Firmata
{
    ...
    class Arduino
    {
        ...
        /// </summary>
        /// <param name="value">Temperature or Humidity.</param>
                    ...
                    lock (this)
                    {
                        ...
                        if (_parsingSysex)
                        {
                            if (inputData == END_SYSEX)
                            {
                                ...
                                if (_storedInputData[0] == STRING_DATA)
                                {
                                    byte[] dht_ReceivedData = new byte[(_sysexBytesRead - 1) / 2];
                                    for (int i = 0; i < dht_ReceivedData.Count(); i++)
                                    {
                                        dht_ReceivedData[i] = (byte)(_storedInputData[(i * 2) + 1] | _storedInputData[(i * 2) + 2] << 7);
                                    }

                                    //Console.WriteLine(dht_ReceivedData.Count());      //for debug
                                    char[] CSEN = new char[dht_ReceivedData.Count()];
                                    for (int j = 0; j < dht_ReceivedData.Count(); j++)
                                    {
                                        CSEN[j] = (char)dht_ReceivedData[j];
                                        //Console.Write((char)dht_ReceivedData[j]);     //for debug
                                    }
        ...
        private const int STRING_DATA = 0x71; //
